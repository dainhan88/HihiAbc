...
const Returnproduct = ({ setShow }) => {
  const { register, handleSubmit } = useForm();
  const handleOnSubmit = (data) => {
    axios.post("/api/returnproduct", data).then((response) => {
  ...
  return (
    <div className="fixed left-0 top-0 bottom-0 right-0 flex items-center justify-center">
      ...
      >
        <h1 className="font-bold text-[20px] mt-10">*Gửi Yêu Cầu Đổi Trả </h1>
        <div className="mt-5">
          <div className="grid md:grid-cols-2 md:gap-6">
            ...
            <div className="relative z-0 mb-6 w-full group">
              <input
                ...
                placeholder=" "
                required
                {...register("soDienThoatKhachDoiTra")}
              />
              <label
